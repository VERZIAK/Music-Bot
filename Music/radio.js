...
module.exports = {
  name: "radio",
  description: "Słuchaj na radiu",
  cooldown: 3,
  edesc: `Wpisz to polecenie, aby odtwarzać radio na żywo!\nUżyj: ${PREFIX}radio <1-34>`,
  async execute(message, args, client) {
    //define the No args Embed, lmao
    let resultsEmbed = new Discord.MessageEmbed()
      .setTitle(`**Dostępne radia**`)//
      .addFields(
        {
...
**34: ** [\`${Radiostations[34 - 1].split(" ")[0]}\`](${Radiostations[34 - 1].split(" ")[1]})`, inline: true
    ...
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Potrzebuje permisji żeby dołaczyć na twój kanał!");
    ...
    if (isNaN(args[0])) {
      ...
      return message.reply(
        new MessageEmbed()
          .setColor("#F0EAD6")
          .setTitle(`Dane radio aktualnie nie jest dostępne, wybierz inne \`1\` oraz \`${Radiostations.length}\``)
      );
    }
    ...
    if (!serverQueue)
      message.channel.send(new MessageEmbed().setColor("#F0EAD6")
        .setDescription(`**👍 Dołączono \`${channel.name}\` 📄 bouned \`#${message.channel.name}\`**`)
        .setFooter(`${message.author.username}#${message.author.discriminator}`));
    //send Search something embed
    message.channel.send(new MessageEmbed().setColor("#F0EAD6")
      .setDescription(`**:radio: Szukanie 🔍 \`${Radiostations[i - 1].split(" ")[0]}\`**`));
    //mute yourself
    ...
    */
    ...
    if (serverQueue) {
      ...
      if (estimatedtime > 60) {
        ...
        estimatedtime = estimatedtime + " Minuty"
      }
      else if (estimatedtime > 60) {
        estimatedtime = Math.round(estimatedtime / 60 * 100) / 100;
        estimatedtime = estimatedtime + " Godziny"
      }
      else {
        estimatedtime = estimatedtime + " Sekundy"
      }
      ...
      const newsong = new MessageEmbed()
        ...
        .addField("Pozostały czas do odtworzenia:", `\`${estimatedtime}\``, true)
    ...
    try {
      ...
      await channel.leave();
      return message.channel.send(`Nie znaleziono kanału: ${error}`).catch(console.error);
    }
    //sende bestätigung
  }
};
