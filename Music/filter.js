...
module.exports = {
  name: "filter",
  description: "Ustaw dzwięki audio",
  aliases: ["f"],
  cooldown: 3,
  edesc: `Wpisz tą komende by dostosować dzwięki audio - style \nUżyj: ${PREFIX}filter <Filtertype>`,
  ...
  async execute(message, args, client) {
    ...
    if (message.channel.activeCollector)
      return attentionembed(message, "Wyszukiwanie jest aktywne!");
    ...
    if (queue && channel !== message.guild.me.voice.channel)
      return attentionembed(message, `Musisz być na tym samym kanale głosowym`);
    ...
    switch (args[0]) {
      ...
      default:
        ...
        .setColor("#F0EAD6")
        .setTitle("Nie jest to prawidłowy filtr, użyj jednego z nich:")
        .setDescription(`
        \`bassboost\`
        \`8D\`
        \`vaporwave\`
        ...
        \`chorus\`
        \`karaoke\`
        \`sofa (sprawia, że ​​dźwięk nadaje się do słuchawek/zestawu słuchawkowego;)\`
        \`desilencer (automatycznie usuwa ciszę w piosence)\`
        \`clear\`   ---  usuwa wszystkie filtry`)
        .setFooter(`Przyklad: ${PREFIX}filter bassboost`)
        )
        break;
    }
    //set choice to zero
