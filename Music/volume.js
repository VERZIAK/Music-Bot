...
module.exports = {
  ...
  cooldown: 5,
  edesc: `Wpisz tą komende, aby zmienić głośność bieżącego utworu <0-200>`,
...
execute(message, args) {
    ...
    if (!queue) return attentionembed(message,`Nie ma nic do słuchania`);
    ...
    if (isNaN(args[0])) return attentionembed(message,"To nie jest numer **0 & 200**");
    //if args is not a Number between 150 and 0 return error
    if (parseInt(args[0]) < 0 || parseInt(args[0]) > 200)
      return attentionembed(message,"To nie jest numer **0 & 200**");
    ...
    .setColor("#F0EAD6")
    .setTitle(`🔊 Zmieniono głośność do: \`${args[0]}%\`!`)  
