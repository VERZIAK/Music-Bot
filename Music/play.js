...
module.exports = {
  name: "play",
  aliases: ["p"],
  description: "Słuchaj muzyki z Youtube/Radia",
  cooldown: 1.5,
  edesc: `Użyj tej komendy jeżeli chcesz zacząć słuchać muzyki.\nUżyj: ${PREFIX}play <TITLE | URL>`,
...
async execute(message, args, client) {
    ...
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return attentionembed(message, `Musisz być na tym samym kanale co ja`);
    //If no args return
    if (!args.length)
    ...
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Potrzebuje permisji, żeby dołączyć na twój kanał!");
    if (!permissions.has("SPEAK"))
      return attentionembed(message, "Potrzebuje permisji do mówienia, żeby grała muzyka");
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Szukam 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Szukam 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
        ...
        //send join message
        message.channel.send(new MessageEmbed().setColor("#F0EAD6")
          .setDescription(`**👍 Dołączono \`${channel.name}\` 📄 bound \`#${message.channel.name}\`**`)
          .setFooter(`By: ${message.author.username}#${message.author.discriminator}`))
        //if its an url
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Dołączono 🔍 [\`LINK\`](${args.join(" ")})**`))
          //if not 
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Dołączono 🔍 \`${args.join(" ")}\`**`))
        }
    ...
    if (urlValid) {
      try {
        ...
        if (error.statusCode === 403) return attentionembed(message, "Prosze odświeżyć!");
        console.error(error);
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .addField("Przewidywany czas do rozpoczęcia muzyki:", `\`${estimatedtime}\``, true)
        .addField("Pozycja w kolejce", `**\`${serverQueue.songs.length - 1}\`**`, true)
    ...
    try {
      ...
      return attentionembed(message, `Nie moge dołączyć na kanał: ${error}`);
    }
  }
};
